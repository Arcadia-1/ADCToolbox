...
for k = 1:length(Tj_list)
    ...
    filename = fullfile("ADCToolbox_example_data", sprintf("sinewave_jitter_%dfs.csv", round(Tj*1e15)));
...
for k = 1:length(clipping_list)
    ...
    filename = fullfile("ADCToolbox_example_data", sprintf("sinewave_clipping_%s.csv", str));
    fprintf("[Save data into file] -> [%s]\n", filename);
    writematrix(data, filename)
end
...
for k = 1:length(kickback_strength_list)
    ...
    filename = fullfile("ADCToolbox_example_data", sprintf("sinewave_kickback_%s.csv", kstr));
    fprintf("[Save data into file] -> [%s]\n", filename);
...
for k = 1:length(glitch_prob_list)
    ...
    filename = fullfile("ADCToolbox_example_data", sprintf("sinewave_glitch_%s.csv", pstr));
    fprintf("[Save data into file] -> [%s]\n", filename);
    writematrix(data, filename)
end
...
for k = 1:length(baseline_list)
    ...
    filename = fullfile("ADCToolbox_example_data", sprintf("sinewave_drift_%s.csv", bstr));
    fprintf("[Save data into file] -> [%s]\n", filename);
    writematrix(data, filename)
end
...
for k = 1:length(ref_error_list)
    ...
    filename = fullfile("ADCToolbox_example_data", sprintf("sinewave_ref_error_%s.csv", rstr));
    fprintf("[Save data into file] -> [%s]\n", filename);
    writematrix(data, filename)
end
...
for k = 1:length(Nyquist_Zone_Fins)
    ...
    filename = fullfile("ADCToolbox_example_data", sprintf("sinewave_Zone%d_Tj_%dfs.csv", ...
        Zone, round(Tj*1e15)));
