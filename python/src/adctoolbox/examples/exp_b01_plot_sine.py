...
# Generate signal
period = 1 / Fin
n_samples = int(n_cycles * period * Fs)
t = np.arange(n_samples) / Fs
signal = A * np.sin(2*np.pi*Fin*t)

print(f"[Sinewave] [Fs={Fs/1e6:.1f}MHz, Fin={Fin/1e3:.1f}kHz, A={A}, cycles={n_cycles}]")

fig, ax = plt.subplots(figsize=(8, 6))
ax.plot(t*1e6, signal, 'o-', markersize=4, linewidth=1.5)
ax.set_xlabel('Time (us)', fontsize=12)
ax.set_ylabel('Amplitude', fontsize=12)
ax.set_title(f'Sine Wave: {n_cycles} Periods', fontsize=14)
