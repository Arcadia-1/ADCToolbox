...
print(f"[Setting] N=[{N_fft}], Fs=[{Fs/1e6:.1f} MHz], Fin_arb=[{Fin_arbitrary/1e6:.1f} MHz], Fin_coh=[{Fin_coherent/1e6:.3f} MHz] (Bin=[{Fin_bin}]) | [Theory] SNR=[{snr_ref:.2f} dB], NSD=[{nsd_ref:.2f} dBFS/Hz]\n")
...
plt.tight_layout()
fig_path = (output_dir / 'exp_s06_spectrum_leakage_demo.png').resolve()
