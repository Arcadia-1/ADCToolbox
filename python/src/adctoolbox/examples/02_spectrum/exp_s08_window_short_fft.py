...
print(f"[Window Comparison - Short FFT]")
print(f"[N_fft] = {N_fft}, [Fs] = {Fs/1e6:.2f} MHz")
print(f"[Fin] = {Fin/1e6:.6f} MHz (Bin {Fin_bin})")
print(f"[Bin width] = {Fs/N_fft/1e3:.1f} kHz (very coarse resolution)\n")
...
plt.tight_layout()
fig_path = output_dir / 'exp_s08_window_short_fft.png'
print(f"\n[Save fig] -> [{fig_path}]")
plt.savefig(fig_path, dpi=150)
plt.close()

print("\n[Observation] Short FFT (N=128) has very coarse frequency resolution")
print("[Trade-off] Wide main lobes consume many bins -> signal power spreads")
print("[Kaiser/Flattop] With side_bin=8/4, may capture too many noise bins")
print("[Result] Even coherent signals show degraded performance with short FFT")
