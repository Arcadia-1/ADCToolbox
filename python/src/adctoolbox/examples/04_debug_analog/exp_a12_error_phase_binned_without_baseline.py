"""Phase-based error analysis: Binned mode WITHOUT baseline
...
# --- 3. Timing: Analysis & Plotting (Binned Mode) ---
t_plot = time.time()

# Analyze and plot results - Binned mode
fig = plt.figure(figsize=(18, 12))
fig.suptitle('Phase Error Analysis (Binned Mode WITHOUT Baseline) - Thermal Noise vs Phase Jitter vs Amplitude Modulation', fontsize=16, fontweight='bold')

# Analyze 3 cases
# Case 1: Thermal Noise Only
ax1 = plt.subplot(3, 2, 1)
analyze_error_by_phase(sig_thermal_only, normalized_freq, data_mode="binned", include_baseline=False, ax=ax1)

# Case 2: Phase Jitter Only
ax2 = plt.subplot(3, 2, 2)
analyze_error_by_phase(sig_pm_only, normalized_freq, data_mode="binned", include_baseline=False, ax=ax2)

# Case 3: Amplitude Modulation Only
ax3 = plt.subplot(3, 2, 3)
analyze_error_by_phase(sig_am_only, normalized_freq, data_mode="binned", include_baseline=False, ax=ax3)
...
fig_path = (output_dir / 'exp_a12_error_phase_binned_without_baseline.png').resolve()
...
fig.suptitle('Phase Error Analysis Comparison: Thermal vs Phase vs Amplitude Noise (Binned Mode WITHOUT Baseline)',
             fontsize=14, fontweight='bold')

# Case 1: Thermal Noise Only
analyze_error_by_phase(sig_thermal_only, normalized_freq, data_mode="binned", include_baseline=False, ax=axes[0])

# Case 2: Phase Noise Only
analyze_error_by_phase(sig_pm_only, normalized_freq, data_mode="binned", include_baseline=False, ax=axes[1])

# Case 3: Amplitude Noise Only
analyze_error_by_phase(sig_am_only, normalized_freq, data_mode="binned", include_baseline=False, ax=axes[2])

plt.tight_layout()
fig_path_3plot = (output_dir / 'exp_a12_error_phase_binned_without_baseline_3plot.png').resolve()
