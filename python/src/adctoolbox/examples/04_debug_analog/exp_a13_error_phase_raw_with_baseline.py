"""Phase-based error analysis: Raw data mode WITH baseline
...
fig.suptitle('Phase Error Analysis (Raw Mode WITH Baseline) - Thermal Noise vs Phase Jitter vs Amplitude Modulation', fontsize=16, fontweight='bold')

# Analyze 3 cases
# Case 1: Thermal Noise Only
ax1 = plt.subplot(3, 2, 1)
analyze_error_by_phase(sig_thermal_only, normalized_freq, data_mode="raw", include_baseline=True, ax=ax1)

# Case 2: Phase Jitter Only
ax2 = plt.subplot(3, 2, 2)
analyze_error_by_phase(sig_pm_only, normalized_freq, data_mode="raw", include_baseline=True, ax=ax2)

# Case 3: Amplitude Modulation Only
ax3 = plt.subplot(3, 2, 3)
analyze_error_by_phase(sig_am_only, normalized_freq, data_mode="raw", include_baseline=True, ax=ax3)
...
fig_path = (output_dir / 'exp_a13_error_phase_raw_with_baseline.png').resolve()
...
fig.suptitle('Phase Error Analysis Comparison: Thermal vs Phase vs Amplitude Noise (Raw Mode WITH Baseline)',
             fontsize=14, fontweight='bold')

# Case 1: Thermal Noise Only
analyze_error_by_phase(sig_thermal_only, normalized_freq, data_mode="raw", include_baseline=True, ax=axes[0])

# Case 2: Phase Noise Only
analyze_error_by_phase(sig_pm_only, normalized_freq, data_mode="raw", include_baseline=True, ax=axes[1])

# Case 3: Amplitude Noise Only
analyze_error_by_phase(sig_am_only, normalized_freq, data_mode="raw", include_baseline=True, ax=axes[2])

plt.tight_layout()
fig_path_3plot = (output_dir / 'exp_a13_error_phase_raw_with_baseline_3plot.png').resolve()
