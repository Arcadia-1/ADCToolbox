...
from adctoolbox import calculate_snr_from_amplitude, snr_to_enob
...
# Ideal case
snr_sweep = calculate_snr_from_amplitude(A_fixed, noise_sweep)
...
for bits, color, ls in zip(adc_bits, colors, linestyles):
    ...
    snr_with_quant = calculate_snr_from_amplitude(A_fixed, noise_total)
...
plt.tight_layout()
fig_path = output_dir / 'exp_b05_snr_calculations.png'
...
for bits in adc_bits:
    lsb = FSR / (2 ** bits)
    quant_noise_rms = lsb / np.sqrt(12)
    max_snr = calculate_snr_from_amplitude(A_fixed, quant_noise_rms)
